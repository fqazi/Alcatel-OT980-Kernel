...
struct msmsdcc_host *host_mmc1 = NULL;
...
#if JRD_DEBUG
	if(c != (cmd_tmp = readl(host->base + MMCICOMMAND)))
	{
		printk("######## [JRD] %s,c %d, cmd_tmp %d", __FUNCTION__, c, cmd_tmp);
		return -1;
	}
#endif
	return 0;
...
msmsdcc_dma_exec_func(struct msm_dmov_cmd *cmd)
{
	...
	if (host->cmd_cmd) {
		while(i++ < 2 && msmsdcc_start_command_exec(host, (u32)host->cmd_cmd->arg, (u32)host->cmd_c))
			msmsdcc_delay(host);
	}
}
...
msmsdcc_start_command(struct msmsdcc_host *host, struct mmc_command *cmd, u32 c)
{
	...
	tmpusehost = host->mmc;
	printk("###### %s %s %d %s %d\n",  __FILE__, __FUNCTION__, __LINE__,  mmc_hostname(host->mmc), cmd->opcode); 
#endif
	msmsdcc_start_command_deferred(host, cmd, &c);
	while(i++ < 2 && msmsdcc_start_command_exec(host, cmd->arg, c))
		msmsdcc_delay(host);
}
...
#if JRD_DEBUG
						printk("###### request_end: %s## %s## %d## %d## %d##\n", 
							__FILE__, __FUNCTION__, __LINE__, cmd->opcode, cmd->error); 
#endif
						timer |=
							msmsdcc_request_end(
							 host, cmd->mrq);
...
#if JRD_DEBUG
static unsigned int test_clock=0;
#endif
static void
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	struct msmsdcc_host *host = mmc_priv(mmc);
	u32 clk = 0, pwr = 0, count = 0;
	int rc;
	...
	if (host->pwr != pwr) {
		host->pwr = pwr;
		//changed by JRD to ensure the power mode is set correctly
		do {
			writel(pwr, host->base + MMCIPOWER);
			if((readl(host->base + MMCIPOWER) & 0x03) != (pwr & 0x03))
				mdelay(2);
			else
				break;
		} while (count++ < 2);
...
/*	[added by JRD]
 *	used by wifi check the sdcc's state:
 * 	1:  sdcc suspend*/
bool msmsdcc_check_state()
{
	if(host_mmc1)
	{
		if(!host_mmc1->clks_on)
			printk("####### [JRD] %s: SUSPEND\n", __FUNCTION__);
		return host_mmc1->clks_on?0:1;
	}
	return 0;
}
EXPORT_SYMBOL(msmsdcc_check_state);
#ifdef CONFIG_PM
static int
...
#if JRD_DEBUG
static ssize_t die_write(struct file *file, const char __user *buf,
						size_t count, loff_t *ppos)
{
		//mmc_go_idle(tmpusehost);
		struct msmsdcc_host *host = mmc_priv(tmpusehost);
		writel(67, host->base + MMCIPOWER);
		return 0;
}

static const struct file_operations die_operations = {
		.open       = nonseekable_open,
		.write      = die_write,
};

static ssize_t die_up(struct file *file, const char __user *buf,
						size_t count, loff_t *ppos)
{
		struct msmsdcc_host *host = mmc_priv(tmpusehost);
		//mmc_power_up(tmpusehost);
		//clk_enable(host->pclk);
//		clk_enable(host->pclk);
//		clk_enable(host->clk);
//		host->plat->translate_vdd(mmc_dev(tmpusehost), 1);
		//printk("+++LIUQIAN+++ pwr %u , test_clk %u \n", host->pwr,test_clock); 
	//	writel(36864, host->base + MMCICLOCK);
		printk("+++LIUQIAN+++ clock: %u, test_clk %u, power: %u, pwr %u,\n",
				readl(host->base + MMCICLOCK), test_clock, readl(host->base + MMCIPOWER), host->pwr);
		return 0;
}

static const struct file_operations pow_operations = {
		.open       = nonseekable_open,
		.write      = die_up,
};
#endif
...
#if JRD_DEBUG
	proc_create_data("sendirq", S_IFREG|S_IRUSR|S_IWUSR, NULL, &die_operations, NULL);
	proc_create_data("powerupmmc", S_IFREG|S_IRUSR|S_IWUSR, NULL, &pow_operations, NULL);
#endif
	return 0;
}
#endif
