 ...
 */
static long
jrd_strtol(const char *nptr, char **endptr, int base)
{
	const char *s;
	long acc, cutoff;
	int c;
	int neg, any, cutlim, errno;
...
static void waswap(struct ether_addr *dst, const struct ether_addr *src)
{
...
static int str2wa(const char *str, struct ether_addr *wa)
{
    ...
    for (i = 0; i < 6; i++) {
	b[i] = (uint8_t) jrd_strtol(ptr, NULL, 16);
	if (i != 5 && !(ptr = strchr(ptr, ':')))
	    ptr = ":00:00:00:00:00";
	    ptr++;
    }

    waswap(wa, (struct ether_addr *) b);

    return 0;
}

static int
dhd_get_jrd_mac_addr(char *jrdmacaddr)
{
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	int ret = 0;
	/*add by stephen*/
	//char jrdmacaddr[18] = "00:11:22:33:44:55";
	char jrdmacaddr[18];
	struct ether_addr wifi_addr = {{0, 0x11, 0x22, 0x33, 0x44, 0xFF}};
	...
	oldfs = get_fs();
	set_fs(get_ds());
	ret = dhd_get_jrd_mac_addr(jrdmacaddr);
	set_fs(oldfs);

	dhd_os_proto_block(dhd);/* for safety, we added this */
	if(ret == 0){
		/*convert wifi address format.*/
		str2wa(jrdmacaddr,&wifi_addr);
#if 0	
		printk("####### [JRD] wifi_addr: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", 
				wifi_addr.octet[0],
				wifi_addr.octet[1],
				wifi_addr.octet[2],
				wifi_addr.octet[3],
				...
				wifi_addr.octet[5]);
#endif
		if (!bcm_mkiovar("cur_etheraddr", (void*)&wifi_addr, ETHER_ADDR_LEN, iovbuf, sizeof(iovbuf))) {
			/* NOTE: avoid deadlock by myself */
			dhd_os_proto_unblock(dhd);
			return -1;
		}	
		printk("[WIFI] : iovbuf = %s \n", iovbuf);
