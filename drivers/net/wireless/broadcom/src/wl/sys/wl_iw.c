...
#ifdef DHD_ADD_WAKELOCK
struct wake_lock wlc_wakelock;
#endif
static int
dev_wlc_ioctl(
	struct net_device *dev,
...
#ifdef DHD_ADD_WAKELOCK
	wake_lock_init(&wlc_wakelock, WAKE_LOCK_SUSPEND, "dev_wlc_ioctl");
	wake_lock(&wlc_wakelock);
#endif
	//added by JRD: Make sure can't send cmd to wifi ic when wifi has been closed
	if(g_onoff == G_WLAN_SET_OFF)
	{
		WL_ERROR(("##### %s, %d\n", __FUNCTION__, __LINE__));
#ifdef DHD_ADD_WAKELOCK
	wake_unlock(&wlc_wakelock);
	wake_lock_destroy(&wlc_wakelock);
#endif
		return -1;
	}
...
//john modified for fit for Android framework
...
//added by john
#ifdef DHD_ADD_WAKELOCK
struct wake_lock wl_off_wakelock;
struct wake_lock wl_on_wakelock;
#endif
...
//added by john
#ifdef DHD_ADD_WAKELOCK
	wake_lock_init(&wl_off_wakelock, WAKE_LOCK_SUSPEND, "sysioc_thread_wl_off");
	wake_lock(&wl_off_wakelock);
#endif

	while (down_interruptible(&wl_ctl->timer_sem) == 0) {

		WL_TRACE(("%s Turning off wifi dev, g_onoff %d\n", __FUNCTION__, g_onoff));
		...
		netif_tx_stop_all_queues(wl_ctl->dev);
		wl_iw_send_priv_event(wl_ctl->dev, "STOP");

		break;
	}

	pid = wl_ctl->sysioc_pid;
	wl_ctl->sysioc_pid = 0;
	WL_TRACE(("######## [JRD] %s: Exited, pid %ld, g_onoff %d\n", __FUNCTION__, pid, g_onoff));

//added by john
#ifdef DHD_ADD_WAKELOCK
	wake_unlock(&wl_off_wakelock);
	wake_lock_destroy(&wl_off_wakelock);
#endif
...
)
{
	...
	WL_TRACE(("Enter %s, pid %ld\n", __FUNCTION__, ctl.sysioc_pid));
	...
	init_timer(&timer);
	timer.expires = jiffies + 10000 * HZ / 1000;
	add_timer(&timer);
...
static int
_wl_control_sysioc_thread_wl_on(void *data)
{
	struct wl_ctrl *wl_ctl = (struct wl_ctrl *)data;
	struct net_device *dev = wl_ctl->dev;

	DAEMONIZE("###### wlcontrol_sysioc_on");

	WL_TRACE(("######  %s: Entered\n", __FUNCTION__));

#ifdef DHD_ADD_WAKELOCK
	wake_lock_init(&wl_on_wakelock, WAKE_LOCK_SUSPEND, "sysioc_thread_wl_on");
	wake_lock(&wl_on_wakelock);
#endif

	while (down_interruptible(&wl_ctl->timer_sem) == 0) {

		WL_TRACE(("###### [JRD] %s: Turning on wifi dev, g_onoff %d\n", __FUNCTION__, g_onoff));
	...
	WL_TRACE(("####### [JRD]%s: Exited, g_onoff %d\n", __FUNCTION__, g_onoff));

#ifdef DHD_ADD_WAKELOCK
	wake_unlock(&wl_on_wakelock);
	wake_lock_destroy(&wl_on_wakelock);
#endif
	complete_and_exit(&wl_ctl->sysioc_exited, 0);
	KILL_PROC(wl_ctl->sysioc_pid, SIGTERM);
}

extern int wifi_shutdown;
static void
wl_iw_on_timerfunc(ulong data)
{
	struct wl_ctrl * wl_ctl = (struct wl_ctrl *)data;

	WL_TRACE(("%s, wifi_shutdown %d\n", __FUNCTION__, wifi_shutdown));
	if(wifi_shutdown != 1)
	{
		mod_timer(wl_ctl->timer, jiffies + 1000 * HZ / 1000);
		return;
	}
	del_timer(wl_ctl->timer);
...
//changed by JRD: To make turn on wifi happened after the wifi has been closed.
static int
wl_iw_control_wl_on(
	struct net_device *dev,
	struct iw_request_info *info
...
{
	int ret = 0;
	static struct wl_ctrl ctl_on;
	static struct timer_list timer_on;

	WL_TRACE(("###### Enter %s\n", __FUNCTION__));
	
	ctl_on.timer = &timer_on;
	ctl_on.dev = dev;
	sema_init(&ctl_on.timer_sem, 0);
	init_completion(&ctl_on.sysioc_exited);
	
	ctl_on.sysioc_pid = kernel_thread(_wl_control_sysioc_thread_wl_on, &ctl_on, 0);

	timer_on.data = (ulong)&ctl_on;
	timer_on.function = wl_iw_on_timerfunc;
	init_timer(&timer_on);
	timer_on.expires = jiffies + 100 * HZ / 1000;
	add_timer(&timer_on);
