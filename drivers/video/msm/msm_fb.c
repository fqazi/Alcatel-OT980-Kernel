...
#include "mdp.h"
#include "mdp4.h"
#include <linux/leds.h> //liuyu added
					...
					enum led_brightness value)
{
	...
	//////////////liuyu added/////////////////////////
	//User input value is between (0 ~ 255)

	static int last_level=100;
	int level=value * 6 / 256;
	if(level!=last_level) //if need to refresh the backlight
	{
		if(last_level==0 && level==1) //means from dark to fade to light
			mdelay(300);

		SN3226SetBacklight(level);

		if(last_level==1 && level==0)  //means enter the suspend
			mdelay(300);
	}
	last_level=level;

	level=15 * (value-0) / (255-0);	
         pmic_keypad_lcd_led_set(level);
